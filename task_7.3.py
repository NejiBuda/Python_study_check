...
class Cell:
    ...
    def make_order(self, cells_res):
        res = " "
        for n in range(int(self.amount / cells_res)):
            res += f"{'📀' * cells_res} \\n"
        res += f"{'📀' * (self.amount % cells_res)}"
        return res

    def __str__(self):
        return f"Result operation: {self.amount * '📀'}" #emoji

    def __add__(self, other):
        return Cell(self.amount + other.amount)

    def __sub__(self, other):
        return self.amount - other.amount if (self.amount - self.amount) > 0 else print("Negative")

    def __mul__(self, other):
        return Cell(int(self.amount * self.amount))

    def __truediv__(self, other):
        return Cell(round(self.amount // self.amount))
